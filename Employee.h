...
public:
   ...
   bool operator() (Employee *pX, Employee *pY) {
	   ...
	   {
		   case NAME:
			   return isAscending_ ? pX->name_ < pY->name_ : pX->name_ > pY->name_;

		   case STATUS:
			   return isAscending_ ? pX->status_ < pY->status_ : pX->status_ > pY->status_;

		   case SALARY:
			   return isAscending_ ? pX->salary_ < pY->salary_ : pX->salary_ > pY->salary_;

		   case PAY_BASIS:
			   return isAscending_ ? pX->payBasis_ < pY->payBasis_ : pX->payBasis_ > pY->payBasis_;

		   case POSITION:
			   return isAscending_ ? pX->positionTitle_ < pY->positionTitle_ : pX->positionTitle_ > pY->positionTitle_;
	   }
   }
private:
